...
parser.add_argument('--prediction_window_size', type=int, default=100,
...
def batchify(data, bsz,data_type=args.data):
    ...
    elif data_type == 'ecg' or data_type =='gesture':
        # Work out how cleanly we can divide the dataset into bsz parts.
...
def fit_norm_distribution_param(args, model, train_dataset, endPoint=10000):
    ...
    for t in range(endPoint):
        ...
        if t >= args.prediction_window_size:
            ...
            errors[t] = torch.FloatTensor(organized[t]) - train_dataset[t][0][1]
            if args.cuda:
